...
- v1
  ...
  - In either case, GPU support for PyTorch and ONNX Runtime is enabled only if your system supports it.
  - If you are not using a GPU on Windows, download the ONNX (cpu, cuda) and PyTorch (cpu, cuda) packages.

- For Windows users, after unzipping the file, run the `start_http.bat` script for your VC.
...
- If connecting remotely, use the `.command` file (Mac) or `.bat` file (Windows) with HTTPS instead of HTTP.
